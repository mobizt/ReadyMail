...
| [`ESP_SSLClient`](https://github.com/mobizt/ESP_SSLClient)       | ✅ | ✅ | ✅ | `Cross-platform, Simplified APIs*, supports PSRAM` |
| WiFiSSLClient       | ✅ | ❌ | ❌ | Lightweight, SSL only |
| WiFiClient          | ❌ | ❌ | ✅ | Plain only |

| Client Type       | Library             | SSL/TLS Support | Recommended For             |
|-------------------|---------------------|------------------|-----------------------------|
| `WiFiClient`      | Built-in            | ❌ (use wrapper) | Basic WiFi boards           |
| `WiFiClientSecure`| ESP8266/ESP32       | ✅               | ESP8266 / ESP32 only        |
| `GSMClient`       | TinyGSM / MKRGSM    | ❌ (use wrapper) | GSM/3G modules              |
| `EthernetClient`  | Ethernet / W5500    | ❌ (use wrapper) | Wired Ethernet              |
| `PPPClient`       | PPPoS / LTE         | ❌ (use wrapper) | Cellular PPP devices        |
| `ESP_SSLClient`   | External library    | ✅               | All platforms (ESP32, STM32, RP2040, etc.) ✅

> ✅ ReadyMail is compatible with all of the above via wrapper or direct integration.
